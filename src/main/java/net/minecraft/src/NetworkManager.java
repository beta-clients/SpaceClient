...
public class NetworkManager {
	...
	public void addToSendQueue(Packet var1) {
		if(!this.isServerTerminating) {
			EventPacketSend e = new EventPacketSend(var1);
	...
	public void addToSendQueueWithoutEvent(Packet var1) {
		if(!this.isServerTerminating) {
			Object var2 = this.sendQueueLock;
			synchronized(var2) {
				this.sendQueueByteLength += var1.getPacketSize() + 1;
	...
	public void processReadPackets() {
		...
		while(!this.readPackets.isEmpty() && var1-- >= 0) {
			...
			EventPacketReceive epr = new EventPacketReceive(var2);
			EventRegistry.handleEvent(epr);
			epr.process(this.netHandler);
		}
