...
public class EffectRenderer {
	...
	public void renderParticles(Entity entity, float f) {
		...
		for(int i = 0; i < 3; ++i) {
			if(this.fxLayers[i].size() != 0) {
				...
				if(i == 1) {
					j = this.renderer.getTexture(VersionManager.version().textures() + "/terrain.png");
				}
			...
			for(int k1 = 0; k1 < j1; ++k1) {
				for(int l1 = 0; l1 < j1; ++l1) {
					for(int i2 = 0; i2 < j1; ++i2) {
						...
						this.addEffect((new EntityDiggingFX(this.worldObj, d, d1, d2, d - (double)i - 0.5D, d1 - (double)j - 0.5D, d2 - (double)k - 0.5D, block, j2, i1)).func_4041_a(i, j, k));
					}
				}
			}
	...
	public void addBlockHitEffects(int i, int j, int k, int l) {
		...
		if(i1 != 0) {
			...
			this.addEffect((new EntityDiggingFX(this.worldObj, d, d1, d2, 0.0D, 0.0D, 0.0D, block, l, this.worldObj.getBlockMetadata(i, j, k))).func_4041_a(i, j, k).func_407_b(0.2F).func_405_d(0.6F));
		}
	}
