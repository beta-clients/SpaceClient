...
public class Block {
	...
	public static final Block bedrock = (new Block(7, 17, Material.rock)).setResistance(1.0F).setStepSound(soundStoneFootstep).setBlockName("bedrock").disableStats();
	...
	public float getBlockBrightness(IBlockAccess var1, int var2, int var3, int var4) {
		return Client.hacks[1].isToggled ? 1.0F : var1.getBrightness(var2, var3, var4, lightValue[this.blockID]);
	}

	public boolean shouldSideBeRendered(IBlockAccess var1, int var2, int var3, int var4, int var5) {
		if(Client.hacks[6].isToggled){
			if(this.blockID == 14 || this.blockID == 15 || this.blockID == 16 || this.blockID == 21 || this.blockID == 56){
				return true;
			}
			else{
				return false;
